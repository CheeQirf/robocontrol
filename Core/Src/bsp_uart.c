...
uint8_t USART4_Start_DmaTx(void)
{
    ...
    if (usart4_tx_dma_current_len == 0 && (usart4_tx_dma_current_len = lwrb_get_linear_block_read_length(&usart4_tx_rb)) > 0)
    {
        __HAL_DMA_DISABLE(&hdma_uart4_tx);

        __HAL_DMA_CLEAR_FLAG(&hdma_uart4_tx, __HAL_DMA_GET_TC_FLAG_INDEX(&hdma_uart4_tx));
        __HAL_DMA_CLEAR_FLAG(&hdma_uart4_tx, __HAL_DMA_GET_HT_FLAG_INDEX(&hdma_uart4_tx));
        __HAL_DMA_CLEAR_FLAG(&hdma_uart4_tx, __HAL_DMA_GET_TE_FLAG_INDEX(&hdma_uart4_tx));
        //        __HAL_DMA_CLEAR_FLAG(&hdma_uart4_tx
        //            , __HAL_DMA_GET_GI_FLAG_INDEX(&hdma_uart4_tx));

        HAL_UART_Transmit_DMA(&huart4, (uint8_t *)lwrb_get_linear_block_read_address(&usart4_tx_rb), (uint16_t)usart4_tx_dma_current_len);
...
void USART4_TxTcCb(UART_HandleTypeDef *huart) // 串口发送的回调函数
{
